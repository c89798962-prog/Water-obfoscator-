...
function buildTrueVMNormal(payloadStr) {
  if (payloadStr.length < 50) return payloadStr; // Script muy corto, sin VM
...
function obfuscateNormal(sourceCode) {
  ...
  // 4. Aplanar control (solo si no hay return/break/goto)
  ...
  // 6. Mangle self-calls (si existe 'self')
...
module.exports = { obfuscate: obfuscateNormal };
