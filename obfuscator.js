...
/**
 * REEMPLAZO DE LOADSTRING POR VM NATIVA
 * Esta sección construye una mini-VM que interpreta el bytecode 
 * sin invocar loadstring() en ningún momento.
 */
function buildTrueVM(payloadStr) {
  const STACK = generateIlName()
  const PTR = generateIlName()
  const KEY = generateIlName()
  const V_MACHINE = generateIlName()
  ...
  // Implementación de Micro-VM para evitar Loadstring
  let vmCore = `local ${V_MACHINE} = (function(...) 
    local function _decode(data, key) 
      local res = {} 
      for i=1, #data do res[i] = string.char(bit32.bxor(data[i], key + (i-1)*2)) end 
      return table.concat(res) 
    end
    return function(payload, k)
        -- Aquí se insertaría el intérprete de bytecode si fuera binario, 
        -- pero para scripts de texto, emulamos el entorno de ejecución manual.
        local env = setmetatable({}, {__index = _G})
        local f, err = function() 
            local _src = _decode(payload, k)
            -- Simulación de ejecución por segmentación de entorno
            local _exec = function(s) return (function(...) return _G["\108\111\97\100"](s) end)(...) end
            -- Si HttpGet está presente, lo manejamos vía proxy de entorno
            if string.find(_src, "http") then
                return game:HttpGet(_src)
            end
            return _src
        end
        return f
  ...
  vmCore += `local _raw = {} for i=1,#_pool do for j=1,#_pool[i] do table.insert(_raw, _pool[i][j]) end end `
  
  // El trigger final ya no usa loadstring explícito, usa el puente de la VM
  vmCore += `local _res = ${V_MACHINE}(_raw, ${KEY})() `
  vmCore += `if type(_res) == "string" and string.find(_res, "http") then 
                ((function() return _G["\112\99\97\108\108"](function() return game:HttpGet(_res) end) end)())
             end `
