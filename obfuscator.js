...
// TÉCNICA 4: Control Flow Flattening (CFF)
...
// TÉCNICA 3: Decoy VMs (Generador de señuelos silenciosos)
function buildDecoyVM() {
  const FAKE_STACK = generateIlName();
  return `local ${FAKE_STACK}={} pcall(function() rawget(getfenv(0), string.char(108,111,97,100,115,116,114,105,110,103))(table.concat(${FAKE_STACK}))() end) `;
}

// TÉCNICA 1: Bytecode Virtual Machine Custom (Expandida con nuevas técnicas)
function buildTrueVM(payloadStr) {
  ...
  // TÉCNICA 2: Llaves Dinámicas (math.pi). 
  // string.byte(tostring(math.pi), 1) es 51 ('3'). Le restamos 51 para que sume 0 al seed original, despistando al análisis estático.
  let vmCore = `local ${STACK}={} local ${KEY}=((string.byte(tostring(math.pi),1))-51)+${heavyMath(seed)} local ${SALT}=${heavyMath(saltVal)} `
  ...
  // TÉCNICA 6: Acceso Críptico a Globales (rawget bypass)
  const ASSERT = `rawget(getfenv(0), ${runtimeString("assert")})`;
  const LOADSTRING = `rawget(getfenv(0), ${runtimeString("loadstring")})`;
  const GAME = `rawget(getfenv(0), ${runtimeString("game")})`;
  const HTTPGET = runtimeString("HttpGet");

  // TÉCNICA 5: Aislamiento por Corrutinas
  let execCode = "";
  if (payloadStr.includes("http")) { 
    execCode = `${ASSERT}(${LOADSTRING}(${GAME}[${HTTPGET}](${GAME}, _e)))() ` 
  } else { 
    execCode = `${ASSERT}(${LOADSTRING}(_e))() ` 
  }
  
  vmCore += `local _co = coroutine.create(function() ${execCode} end) coroutine.resume(_co) `
  return vmCore
}

// TÉCNICA 7: Capas de Envoltura (Metamorphic Wrappers rotativos combinados con TÉCNICA 3: Señuelos)
function buildSingleVM(innerCode, handlerCount) {
  ...
  for (let i = 0; i < handlers.length; i++) {
    if (i === realIdx) { 
        ...
        // En lugar de "return nil", inyectamos un Decoy VM para despistar depuradores.
        let decoy = buildDecoyVM();
        out += `local ${handlers[i]}=function(lM) local lM=lM; ${generateJunk(3)} ${decoy} return nil end ` 
    }
  }
  out += `local ${DISPATCH}={`
